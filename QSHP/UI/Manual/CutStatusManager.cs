...
namespace QSHP.UI.Manual
{
    public partial class CutStatusManager : BaseForm
    {
        ...
        private void CuttingManager_Load(object sender, EventArgs e)
        {
            ...
            preTime.Text = TimeSpan.FromSeconds(Group.PreTime + Globals.DevCfg.CuttingTime+chs*10).ToString(@"hh\:mm\:ss");
                ...
                case ProcessCmd.CutPauseCmd:
                    {
                        ...
                        if (ParentForm != null)
                        {
                            ParentForm.PushChildForm(new CutPauseManager(group));//进入暂停界面
                        }
                    } break;
